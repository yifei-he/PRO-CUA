...
def auto_eval_by_gpt4v(process_dir, openai_client, api_model, img_num):
    ...
    ans_info = it_messages[-1]["content"]['parts'][0]
    if it_messages[-1]['message_type'] != 'final_answer':
        print('Not find answer for ' + process_dir)
        print()
        return 0
    # ans_info = it_messages[-1]["content"][0]['text']
    # if it_messages[-1]['type'] != 'message':
    #     print('Not find answer for ' + process_dir)
    #     print()
    #     return 0
...
def main():
    ...
    if os.path.exists(f'{main_folder}/results.json'):
        print(f'Found existing results.json in {main_folder}, will continue to append results')
        with open(f'{main_folder}/results.json', 'r', encoding='utf-8') as f:
            results = json.load(f)
        subfolders = [d for d in subfolders if d.split('/')[-1] not in results]
    else:
