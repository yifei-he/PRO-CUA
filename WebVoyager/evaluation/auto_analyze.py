...
The action space of the agent is 
...
For the tasks where the model does not even output an answer, namely the result response is 'NULL', it is because it fails to finish the task within the preset maximum steps, and you should return 'NO ANSWER'. Under those cases, you should analyze the failure mode of the model, by first decomposing the task into sub-goals, then pointing out what action or thought process of the agent are incorrect in achieving those sub-goals, finally providing corrections on those steps on how to complete this task correctly. Note that the agent can only take the aformentioned actions. Focus on abnormal behavioral patterns that make the agent stuck, including but not limited to repeated but useless actions, failure to deal with pop-up windows, failure to report the results, etc. Specify the useless actions that the agent repeatedly performs, for instance, 'the agent keep scrolling up and down the pages without gaining informaiton'. Use bullet points to summarize the analysis."""
