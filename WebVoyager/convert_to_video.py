...
def convo_to_video(convo, image_folder, video_name="output.mp4", frame_folder="temp_figs"):
    ...
    for i in range(total_len):  # Process messages in pairs
        ...
        if i < len(messages):# and messages[i]["role"] == "user":
            # try:
            #     img_base64 = messages[i].content.screenshot.payload
            #     if img_base64:
            ...
            #     pass
            try:
                img_pil = Image.open(f'{image_folder}/screenshot{i}.png')
            # if no file found, make img_pil a blank image
            except FileNotFoundError:
                print(f"Warning: screenshot{i//2+1}.png not found, using the previous image.")
                pass
        ...
        if i < len(messages) and messages[i]["role"] == "assistant":
            try:
                cmd = messages[i]['content']
                # cmd = str(messages[i].content)
                # if "computer" in messages[i].recipient:
                #     cmd = f"{str(messages[i].role)} -> {str(messages[i].recipient)} :" + cmd
            except AttributeError:
                pass
...
# traj_dir = '/mnt/italynorth/prm-cache/users/t-yifeihe/cua/results/webvoyager_notime/uitars_50steps_15imgs/20250608_19_05_05/taskBBC News--22'
traj_dir = '/mnt/cache/users/t-yifeihe/cua/results/WebVoyager_data_clean/uitars_50_5_parallel_playwright/20250704_22_39_31/taskBBC News--41'

with open(f"{traj_dir}/interact_messages.json", "r", encoding="utf-8") as f:
    convo = json.load(f)

convo_to_video(
    convo, image_folder=traj_dir, video_name=f"{traj_dir}/video/bbc-41.mp4", frame_folder=f"{traj_dir}/video"
)  

# convo_to_video(
#     convo, image_folder=traj_dir, video_name="results/webvoyager/videos/gpt-4o_som/20250604_18_18_34/taskAllrecipes--5/output.mp4", frame_folder="results/webvoyager/videos/gpt-4o_som/20250604_18_18_34/taskAllrecipes--5"
# )  