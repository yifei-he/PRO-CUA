...
n_epochs=2
dataset=webvoyager
FULL_NAME=${EXP_NAME}_${MODEL_NAME}_${dataset}_${MAX_TASKS}tasks_${MAX_STEPS}steps_${DATA_MODE}_temperature${temperature}_${n_epochs}epochs_${BASE_LR}
CKPT_ROOT=${PRO_CUA_CKPT_ROOT}/qwen3-vl-4b
...
cd "$WEBVOYAGER_ROOT"
if [ "$round" -eq 1 ]; then
  main_folder=${PRO_CUA_DATA_ROOT}/qwen3_WebVoyager_train_data_unique_ids_train/qwen3_4b_512_tasks/round_1
...
if [ "$round" -eq 1 ]; then
  ...
  model_name_or_path=${CKPT_ROOT}/${FULL_NAME}_round_$((round-1))
fi
...
llamafactory-cli train examples/train_full/qwen3vl_full_sft.yaml \
  ...
  output_dir=${CKPT_ROOT}/${FULL_NAME}_round_${round} \
...
rm -rf "${CKPT_ROOT}/${FULL_NAME}_round_$round"/checkpoint-*

model_path=${CKPT_ROOT}/${FULL_NAME}_round_$round
