...
from gpt_prompts import GPT_STEP_JUDGE, GPT_THOUGHT_AUGMENTATION, USER_PROMPT, GPT_STEP_JUDGE_REVISED
...
def _call_api_with_retry(client, fallback_client, model_name, messages, process_dir, call_type="thought", model_idx=0):
    ...
    while True:
        ...
        try:
            if model_name == 'gpt-4o':
                ...
                cur_model_idx = model_idx % (len(api_models) + 1)
                
                # if client_type == 'fallback':
                if cur_model_idx == len(api_models):
                    cur_model = 'llmclient_fallback'
                    response = fallback_client.chat_completions_create(
                        model='dev-o4-mini-2025-04-16', messages=messages, seed=42
                    )
                else:
                    cur_model = api_models[cur_model_idx]
                    response = client.chat.completions.create(
            ...
            if call_type == "judge":
                ...
                if score is not None:
                    ...
                    print(f"Warning: No valid score found in judge response, retrying... (attempt {retry}/20) for {process_dir} with model {cur_model}")
            ...
            print(f"{call_type.capitalize()} API call failed ({client_type}): {e} with {cur_model} on {process_dir} (attempt {retry}/20)")
            # if model_name == 'o4-mini':
            #     client_type = 'azure' if client_type == 'fallback' else 'fallback'
    ...
    for message in it_messages:
        if 'message_type' in message and message['message_type'] in ['action', 'final_answer']:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                model_idx = random.randint(0, 2)
                # Submit thought and judge calls to run in parallel
                future_thought = executor.submit(
                    _call_api_with_retry,
                    openai_client, fallback_client, 'o4-mini', messages, process_dir, "thought", model_idx
                )
                # model_idx = random.randint(0, 2)
                # future_judge = executor.submit(
                #     _call_api_with_retry,
                #     openai_client, fallback_client, 'gpt-4o', judge_messages, process_dir, "judge", model_idx
                # )
                model_idx = random.randint(0, 2)
                future_judge = executor.submit(
                    _call_api_with_retry,
                    openai_client, fallback_client, 'o4-mini', judge_messages, process_dir, "judge", model_idx
                )
...
def main():
    ...
    # filtered_folders = ['/mnt/cache/users/t-yifeihe/cua/data/openwebvoyager_webvoyager_difficult_gpt4o_cua_4rollout_pass_lt_3_round3/20250723_18_25_05/taskCoursera--extend--30-1']
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        future_to_folder = {
            executor.submit(auto_eval_by_gpt4v, folder, client, 'o4-mini'): folder
