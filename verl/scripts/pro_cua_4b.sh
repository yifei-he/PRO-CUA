...
MODEL_NAME=qwen3_4b
MAX_TASKS=256
FULL_NAME=${EXP_NAME}_${MODEL_NAME}_${MAX_TASKS}tasks_qwen3-4b-prm_all_steps_finished_traj_verl_${BASE_LR}
...
#######################################################
if [ "$round" -eq 1 ]; then
folder_path=${PRO_CUA_DATA_ROOT}/qwen3_WebVoyager_train_data_unique_ids_train/qwen3_4b_512_tasks_20steps_temperature0.7_4b_512_tasks/round_1
...
cd "$WEBVOYAGER_ROOT"
python construct_dataset.py --algo step_grpo_prm_all_steps_finished_traj --correct_step_only --finished_traj_only --round $round --main_folder $folder_path --judge_file_name interact_messages_dedup.json --max_tasks ${MAX_TASKS}
conda deactivate

conda activate verl
cd "$VERL_ROOT"
python "$VERL_ROOT/examples/data_preprocess/webvoyager_step_to_parquet.py" --input_json ${folder_path}/step_grpo_prm_all_steps_finished_traj_round_${round}.json --output_dir ${folder_path}/verl
# fi
...
ckpt_path=${PRO_CUA_CKPT_ROOT}/qwen3-vl-4b/${FULL_NAME}/round_$((round-1))
...
MODEL_PATH=${PRO_CUA_CKPT_ROOT}/qwen3-vl-4b/${FULL_NAME}/round_$((round-1))/global_step_${LAST_STEP}/actor_hf
fi
...
python3 -m verl.trainer.main_ppo \
  ...
  reward.reward_model.model_path=Qwen/Qwen3-VL-4B-Instruct \
  ...
  reward.custom_reward_function.path=${VERL_ROOT}/examples/data_preprocess/webvoyager_reward_new.py \
  reward.custom_reward_function.name=compute_score_prm \
  ...
  trainer.default_local_dir=${PRO_CUA_CKPT_ROOT}/qwen3-vl-4b/${FULL_NAME}/round_${round}
...
########################################################
ckpt_path=${PRO_CUA_CKPT_ROOT}/qwen3-vl-4b/${FULL_NAME}/round_${round}
...
cd step_eval
python filter_repeat_qwen3.py --process_dir "${PRO_CUA_DATA_ROOT}/qwen3_WebVoyager_train_data_unique_ids_train/${FULL_NAME}/round_$((round+1))"
