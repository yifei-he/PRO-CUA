...
_FONT_CANDIDATES = [
    os.path.join(_SCRIPT_DIR, "NotoSerifSC-SemiBold.otf"),
    os.path.join(os.path.dirname(_SCRIPT_DIR), "NotoSerifSC-SemiBold.otf"),
]
...
def scale_coordinate(coord, image_w, image_h):
    if not coord or len(coord) < 2:
        return None
    return (
        int(coord[0] / 1000 * image_w),
        int(coord[1] / 1000 * image_h),
    )


def qwen3_tool_call_to_action_str(tool_call, image_w, image_h):
    if not tool_call or tool_call.get("name") != "computer_use":
        return None
    ...
    coord = scale_coordinate(args.get("coordinate"), image_w, image_h)
    end_coord = scale_coordinate(
        args.get("end_coordinate") or args.get("coordinate_end") or args.get("end_point"),
        image_w,
        image_h,
    )

    def point_tag(point):
        return f"<point>{point[0]} {point[1]}</point>"

    if action == "left_click":
        return f"click(point='{point_tag(coord)}')" if coord else "click()"
    if action == "right_click":
        return f"right_single(point='{point_tag(coord)}')" if coord else "right_single()"
    if action in ["double_click", "triple_click"]:
        return f"left_double(point='{point_tag(coord)}')" if coord else "left_double()"
    if action == "middle_click":
        return f"click(point='{point_tag(coord)}')" if coord else "click()"
    if action == "mouse_move":
        return f"move(point='{point_tag(coord)}')" if coord else "move()"
    if action == "left_click_drag":
        start_point = coord
        end_point = end_coord or coord
        if start_point and end_point:
            return (
                f"drag(start_point='{point_tag(start_point)}', "
                f"end_point='{point_tag(end_point)}')"
            )
        return "drag()"
    if action == "scroll":
    ...
    if action == "hscroll":
        ...
        return f"scroll(direction='{direction}')"
    if action == "type":
        text = normalize_action_text(args.get("text"))
    ...
    if action in ["answer", "terminate"]:
        text = normalize_action_text(args.get("text") or args.get("status") or "")
        return f"finished(content='{text}')"

    return None


def process_single_image(action_str, image, ins_cmd="", color=(255, 48, 48), from_eval=False):
    """
    Annotate a single in-memory screenshot without any file reads or writes.

    `action_str` may be either:
    - a rendered action string such as `click(point='<point>10 20</point>')`
    - an assistant response / raw Qwen3 tool-call string containing `<tool_call>`

    Returns:
        (annotated_image, action_centers)
    """
    if not isinstance(image, Image.Image):
        raise TypeError("image must be a PIL.Image.Image instance")
    if not isinstance(action_str, str) or not action_str.strip():
        raise ValueError("action_str must be a non-empty string")

    resolved_action_str = action_str.strip()
    if "<tool_call>" in resolved_action_str:
        tool_call = extract_tool_call(resolved_action_str)
        resolved_action_str = qwen3_tool_call_to_action_str(
            tool_call,
            image.width,
            image.height,
        )
        if not resolved_action_str:
            raise ValueError("Unable to convert tool call to action string")

    return actions_visual(
        [resolved_action_str],
        image.copy(),
        ins_cmd,
        color=color,
...
def actions_visual(action_group: list, pil_img: Image, ins_cmd: str, color=(255, 48, 48), from_eval=False):
    ...
    for i, action_str in enumerate(action_group):
        ...
        if action_type in ["click", "right_single", "left_double", "move"]:
            if 'point' in args:
...
def process_folder_visualization(file_dir):
    trajectory_path = os.path.join(file_dir, "interact_messages.json")
    if not os.path.exists(trajectory_path):
        print(f"Skipping {file_dir} as output.json does not exist.")
        return
    ...
    for i, action in enumerate(trajectory):
        ...
        pil_img = Image.open(os.path.join(file_dir, f"screenshot{action_id+1}.png"))
        tool_call = extract_tool_call(action.get('content'))
        action_str = qwen3_tool_call_to_action_str(
            tool_call,
            pil_img.width,
            pil_img.height,
        )
        if not action_str:
            continue
        action_group = [action_str]

        vis_img, action_centers = actions_visual(action_group, pil_img.copy(), ins_cmd)
        vis_img.save(os.path.join(file_dir, 'annotated_screenshots', f"screenshot{action_id}.png"))
        
        # Create zoomed image if there's an action center
        # if action_centers:
        #     # Use the first action center if multiple actions
        #     action_center_info = action_centers[0]
        #     zoomed_img = create_zoomed_action_image(pil_img, action_center_info, zoom_size=200)
...
def render_single_image_demo(image_path, output_path, action_str=None, assistant_response=None, ins_cmd=""):
    if not action_str and not assistant_response:
        raise ValueError("Provide either action_str or assistant_response")

    resolved_action = assistant_response if assistant_response else action_str
...
if __name__ == '__main__':
    ...
    parser.add_argument('--action_str', type=str, default=None)
    parser.add_argument('--assistant_response', type=str, default=None)
    parser.add_argument('--instruction', type=str, default='')
    args = parser.parse_args()

    if args.image_path:
        if not args.output_path:
            raise ValueError("--output_path is required when using --image_path")
        render_single_image_demo(
            image_path=args.image_path,
            output_path=args.output_path,
            action_str=args.action_str,
            assistant_response=args.assistant_response,
            ins_cmd=args.instruction,
        )
        raise SystemExit(0)

    main_folder = args.process_dir
    subfolders = [f.path for f in os.scandir(main_folder) if f.is_dir()]
    # subfolders = ['/data/common/yifei/cua/data/qwen3_WebVoyager_train_data_unique_ids_train/round_1/taskAllrecipes--12146-1']
...
# python /home/yifeihe/cua_step_rl/verl/examples/data_preprocess/data_visualization.py \
#   --image_path /scratch/yifeihe/cua/data/qwen3_WebVoyager_train_data_unique_ids_train/qwen3_4b_512_tasks/round_1/taskAmazon--298-1/screenshot2.png \
#   --output_path /home/yifeihe/cua_step_rl/verl/examples/data_preprocess/annotated.png \
#   --action_str "click(point='<point>320 480</point>')" \
